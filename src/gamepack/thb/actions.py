...
def user_input_action(initiator, action, actors, categories, candidates, trans=None):
    # initiator: Action or EH requesting this
    # action: pending action, eg: lambda p, cl, pl: act(p, cl, pl)
...
class DummyAction(Action):
    def __init__(self, source, cards, target_list):
        self.source = source
        self.cards = cards
...
def ask_for_action(initiator, actors, categories, candidates, trans=None):
    act = user_input_action(initiator, DummyAction, actors, categories, cadidates, trans)
    if not act:
        return None, None

    if act.cards or act.target_list:
        return act.source, (act.cards, act.target_list)

    return act.source, None
...
def ask_for_drop(initiator, actor, categories):
    ...
    return user_input_action(initiator, action, [actor], categories, [])
...
class UseCard(UserAction):
    ...
    def apply_action(self):
        ...
        drop = user_input_action(self, action, [target], ('cards', 'showncards'), [])
