...
class Game(GameObject, Generic[A, EH]):
    ...
    def user_input(
        ...
        return self.runner.user_input(entities, inputlet, timeout, type, trans)
    ...
    def process_action(self, action: A) -> bool:
        ...
        if action.done:
            ...
            try:
                ...
                hybrid = (
                    self.hybrid_stack
                )  # noqa, when crashes pytest will show hybrid stack here by inspecting local variables
...
class EventHandler(GameObject):
    ...
    def get_interested(self):
        ...
        assert isinstance(interested, (list, tuple)), (
            "Should specify interested events! %r" % self.__class__
        )
    ...
    def make_list(g, eh_classes, fold_arbiter=True):
        ...
        for cls in eh_classes:
            assert not issubclass(cls, EventArbiter), (
                "Should not pass arbiters in make_list, %r" % cls
            )
        ...
        for eh in table.values():
            eh.execute_before = (
                set(eh.execute_before) & allnames
            )  # make it instance var
        ...
        lst.sort(
            key=lambda v: v.__class__.__name__
        )  # must sync between server and client
        ...
        while lst:
            ...
            if not commit:
                raise GameError(
                    "Can't resolve dependencies! Check for circular reference!"
                )
    ...
    def _dump_eh_dependency_graph():
        ehs: Set[Type[EventHandler]] = {
            i for i in all_gameobjects if issubclass(i, EventHandler)
        }
        ...
        with open("/dev/shm/eh_relations.dot", "w") as f:
            f.write("digraph {\nrankdir=LR;\n")
            f.write("\n".join(["%s -> %s;" % (a, b) for a, b in dependencies]))
            f.write("}")
...
class EventDispatcher(GameObject):
    ...
    def emit(self, evt_type: str, data: Any):
        ...
        if log.isEnabledFor(logging.DEBUG):
            if isinstance(data, (list, tuple, str)) or hasattr(
    ...
    def _get_relevant_eh(self, tag: str):
        ...
        ehs = [eh for eh in self._event_handlers if tag in eh.get_interested()]
...
def sync_primitive(val: T_sync, to: Union[Player, BatchList[Player]]) -> T_sync:
    ...
    if isinstance(val, list):
        ...
        to.reveal(lst)
        rst = val.__class__(i.value for i in lst)
    else:
        v = SyncPrimitive(val)
        to.reveal(v)
        rst = v.value
