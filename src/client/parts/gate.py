...
class UnityUIEventHook(EventHandler):
    ...
    def evt_user_input(
        self, arg: Tuple[InputTransaction, Inputlet]
    ) -> Tuple[InputTransaction, Inputlet]:
        evt = Event()
    ...
    def handle(self, evt: str, arg: Any) -> Any:
        ...
        if evt == "user_input":
            ...
            trans, ev, data = arg
            core.gate.post(
                f"game.input.trans:feedback:{ev}",
                {
                    "id": id(trans),
                    **data,
                },
...
class Gate(object):
    ...
    def post(self, op: str, data: Any) -> None:
        if not self.connected:
            log.debug(
                "Attempt to post message when gate is not conected: %s -> %s", op, data
            )
            return
    ...
    def do_call(self, call: CoreCall) -> None:
        ...
        if cid := call.get("call_id"):
            ...
            try:
                ...
                raise Exception(
                    f"Gate: error serializing result: call = {call}, result = {ret}"
                ) from e
    ...
    def on_observe_started(self, v: Tuple[Game, int]):
        core = self.core
        g = v[0]
        self.post(
            "observer_started",
            {
                "game": core.game.gid_of(g),
    ...
    def on_player_presence(
        self, v: Tuple[Game, List[Tuple[int, wire.PresenceState]]]
    ) -> Any:
        core = self.core
        g, presence = v
        self.post(
