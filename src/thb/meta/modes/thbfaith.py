...
class THBattleFaith(UIMetaBare):
    ...
    description = (
        ...
        '\n'
        '|G角色选择|r：共发给每人4张角色牌，其中一张为暗置。每名玩家选择其中一张作为出场角色，再选择一张作为备用角色（不得查看暗置角色牌）。每方阵营将三张备用角色牌置于一旁作为备用角色。\n'
        '\n'
        '|G游戏开始|r：游戏开始时，所有角色摸4张牌，此时除首先行动的玩家均可以进行一次弃置4张牌并重新摸4张牌的操作。\n'
        '\n'
        '|G角色坠机|r：当一名角色被击坠时，弃置该角色全部区域内的所有牌。从剩余的备用角色中选择一名作为出场角色并明示之，之后摸4张牌。此时该角色可以弃置全部的4张牌并重新摸4张牌。击坠不执行任何奖惩。\n'
        '\n'
