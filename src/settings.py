...
|B最近更新情况：|r
平衡性调整：
早苗/神裔: 效果由原来的2选1变成了重铸一张牌
正邪/挑拨: 目标角色无法再指定自己为弹幕的目标，并移出了 KOF 模式。
水桥/嫉妒: 现在不需要将城管执法进行转化也可触发嫉妒。
因幡帝: 移出了 KOF 模式。
罪袋的头套: 触发的判定条件调整为和罪袋一致。
针妙丸/付丧神之怨: 时机调整为失去装备区的牌后发动

重做了博丽灵梦和犬走椛。

新元素： BOSS技：在身份模式下，BOSS可以获得额外的技能。少部分角色有专属BOSS技。

干劲：我们引入了这个新的机制，来代替之前“使用弹幕次数限制”的概念
即从之前的“出牌阶段限使用一次弹幕”改为“出牌阶段使用弹幕需要消耗一点干劲”
同理，它也将作为一些其它技能的 cost（比如魔导书）。

武器重铸：为武器增加了重铸的选项。现在你可以选择是重铸或是装备一把武器了。该选项需要消耗一点干劲。

更新了大量的立绘，以及角色描述，使其更规范和易于理解。
